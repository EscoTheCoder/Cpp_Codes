...
public:
    Classical_PlayList(string inPlaylistName, string inPlaylistDescription, string in_period);
    friend ostream &operator<< (ostream &left, const Classical_PlayList &right);
private:
    string period;
};
...
Classical_PlayList::Classical_PlayList(string inPlaylistName, string inPlaylistDescription, string in_period) :
    PlayList(inPlaylistName, inPlaylistDescription) {
    period=in_period;
}

ostream &operator<< (ostream &left, const Classical_PlayList &right){
    left<<"["<<right.playlist_name<<", "<<right.playlist_description<<", "<<right.period<<", "<<right.xronikh_diarkeia_playlist<<" : "<<endl;
    for(int i=0; i<right.Videos_Size; i++){
        left<<endl<<right.Videos[i];
    }
    left<<endl<<endl<<"]";
...
int main() {
    ...
    Classical_PlayList classical_playlist("My PlayList", "The Best Songs","2024");

    Video video;

    video.set_Video("Sin Boy","Cherry",207); //207 = defterolepta
    classical_playlist.add_video(video);
